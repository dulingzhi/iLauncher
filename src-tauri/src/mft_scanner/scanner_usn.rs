...
impl UsnScanner {
    ...
    fn query_usn_journal(&self, volume_handle: HANDLE) -> Result<UsnJournalData> {
        ...
        unsafe {
            ...
            if !result.as_bool() {
                ...
                error!("❌ FSCTL_QUERY_USN_JOURNAL failed with error: {:?}", error);
                
                // 如果USN Journal不存在，尝试创建
                if error.0 == 0x80070490u32 as i32 { // ERROR_JOURNAL_NOT_ACTIVE
                    info!("   USN Journal not active, attempting to create...");
                    return self.create_usn_journal(volume_handle);
                }
                
                return Err(anyhow::anyhow!("Failed to query USN Journal: {:?}", error));
            }
        }
    ...
    fn create_usn_journal(&self, volume_handle: HANDLE) -> Result<UsnJournalData> {
        ...
        unsafe {
            ...
            if !result.as_bool() {
                let error = GetLastError();
                return Err(anyhow::anyhow!("Failed to create USN Journal: {:?}", error));
            }
            ...
            unsafe {
                ...
                if !result.as_bool() {
                    ...
                    if error.0 == 38 { // ERROR_HANDLE_EOF
                        ...
                        warn!("   DeviceIoControl iteration {} failed: {:?}", iteration, error);
                        break;
                    }
                }
