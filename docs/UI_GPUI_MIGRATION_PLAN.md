...
| P4 切换与退役 | ✅ 完成 | 打包链路（8d54c5e：NSIS + minisign + CI，静默装/卸实测）；React 前端（1325c5b）、src-tauri（4c4b1eb）、v2 索引链路（IndexBuilder/IndexQuery/PathReader/UsnIncrementalUpdater/DeltaMerger/QueryCache/MultiDriveScanner + MftFileEntry 等 6 模块）全部退役；验收达成：删除后 ilauncher-index 34 测 + gpui-app 四组合 125/126 测全绿、clippy 0（含顺手修复 index_v2 存量 11 条 clippy 警告，fst/roaring/smartstring/once_cell/serde/serde_json 六个死依赖移除） |
